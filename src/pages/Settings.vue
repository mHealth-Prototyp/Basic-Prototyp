  ...
  <q-card class="settings-card">
    ...
    <q-card-section>
      <form>
        ...
        <ul class="oid-list">
          <li><b>{{$t('settings.oidsMpi')}}:</b> <br /> {{ oids.mpiId }}</li>
          <li><b>{{$t('settings.oidsSpid')}}:</b> <br /> {{ oids.eprSpid }}</li>
          <li><b>{{$t('settings.oidsAhv')}}:</b> <br /> {{ oids.ahv }}</li>
          <li><b>{{$t('settings.oidsApp')}}:</b> <br /> {{ oids.app }}</li>
        </ul>
...
export default defineComponent({
  ...
  methods: {
    ...
    reset(e: Event | undefined, getDefault: boolean): void {
      ...
      this.orgOid = (
        fhirpath.evaluate(settings.organization, 'Organization.identifier.where(system.contains(\'urn:oid\')).system') as string[]
      )[0];
      ...
      this.practiceSetting = PRACTICE_SETTING_CODES.find(psc => psc.defaultCoding.code === settings.practiceSetting.code);
    }
  },
  watch: {
...
.button-container .q-btn, #logout-button {
  margin: 0.5em;
}
</style>
