...
<div v-if="translations">
  <q-table :title="translations.titleLabel"
           :loading="loadingData"
...
import { generateRandomPatient } from 'src/utils/patientUtils';
import { useITI83 } from 'src/utils/epdPlaygroundUtils';
...
export default defineComponent({
    ...
    return {
      ...
      tableColumns: [                       // defines the columns of the result table
        ...
        },
        {
          ...
          align: 'left',
          field: (row: {pat: Patient}) => {
            const identifier = row.pat.identifier?.find(i => i.system === this.$store.getOids().ahv);
            return identifier ? identifier.value : '';
          }
        }
      ]
  ...
  beforeMount() {
    ...
    if (!this.$props.patients) {
      const patientsFromStore = this.$store.getLocalPatients();
      if (patientsFromStore.length > 0) {
        preparePatients = patientsFromStore;
      } else {
        for (
          let i = 0;
          ...
          preparePatients.push(generateRandomPatient(this.$store.getOrganizationResource(), this.$store.getOids()));
        }
      }
    }

    this.$store.setLocalPatients(preparePatients);
  ...
  methods: {
    ...
    hasEprSpid(pat: Patient): Promise<boolean> {
      ...
      if (localIdentifier && localIdentifier.system && localIdentifier.value) {
        ...
        return useITI83(localIdString, [this.$store.getOids().eprSpid])
        .then(() => {
          return true;
        })
        .catch(() => {
    ...
    filterTable(data: {pat: Patient, hasEpr: boolean}[], filter: string): {pat: Patient, hasEpr: boolean}[] {
      ...
      return data.filter(row => {
        ...
        const ahv = patient.identifier?.find(i => i.system === this.$store.getOids().ahv)?.value || '';
