...
<div v-if="translations" >
  ...
  <form>
    <div class="row">
      <div class="col">
        <q-input v-model="familyName"
                 type="text"
                 :label="translations.nameInputLabel" />
      </div>
      <div class="col">
        <q-input v-model="givenName"
                 type="text"
    ...
    <q-btn id="search-button"
           @click="search"
           type="submit"
           :label="translations.searchButtonLabel"
           :disable="!enoughParameters || loading"
           :loading="loading">
    <q-tooltip class="bg-warning" v-if="!enoughParameters">{{translations.notEnoughParameter}}</q-tooltip>
    </q-btn>
  </form>
  </div>
  <p v-if="notFound" class="warning">{{translations.noPatientFound}}</p>
  <q-table :title="translations.resultTableTitle"
...
import { iti78Params, useITI78 } from 'src/utils/epdPlaygroundUtils';
    ...
    return {
      ...
      tableColumns: [   // defines the columns of the result table
        ...
        },
        {
          ...
          align: 'left',
          field: (pat: Patient) => {
            const identifier = pat.identifier?.find(i => i.system === this.$store.getOids().eprSpid);
