...
<div>
  <div class="loading"
    ...
    <q-linear-progress :value="fetchingProgress"
                       class="q-mt-md" />
  ...
  <div v-else>
    <q-table :title="translations.titleLabel + mpiID.value"
             dense
             :rows="documents"
    ...
    >
      <template v-slot:top-right>
        <q-input borderless dense debounce="300" v-model="tableFilter" :placeholder="translations.searchLabel">
...
import { getClassCodeString, getFacilityClassCodeString, getTypeCodeString } from 'src/utils/fhirUtils';
import { defineComponent, PropType } from 'vue';
import { useITI67, useITI68, useITI83 } from '../utils/epdPlaygroundUtils';
...
// needed to use DocumentSearch as ref, to have parent access to functions and data
export type DocumentSearchRef = InstanceType<typeof DocumentSearch>;
...
const DocumentSearch =  defineComponent({
    ...
    return {
      ...
      tableColumns: [                                   // definitions of the result table rows
        ...
        },
        {
          ...
          format: (coding: Coding) => {
            if (coding && coding.code) {
              const translated = getClassCodeString(coding.code, getLangStringFromLocale(this.$props.locale || 'de-CH'));
        ...
        },
        {
          ...
          format: (coding: Coding) => {
            if (coding && coding.code) {
              const translated = getTypeCodeString(coding.code, getLangStringFromLocale(this.$props.locale || 'de-CH'));
              return translated === '?' && coding.display
                ? this.shortenString(coding.display) as string
                : this.shortenString(translated) as string
            }
        ...
        },
        {
          ...
          format: (coding: Coding) => {
            if (coding && coding.code) {
              const translated = getFacilityClassCodeString(coding.code, getLangStringFromLocale(this.$props.locale || 'de-CH'));
              return (translated === '?' && coding.display)
                ? this.shortenString(coding.display) as string
                : this.shortenString(translated) as string
            }
  ...
  methods: {
    ...
    fetchData() {
      ...
      let localId = this.$props.patient.identifier?.find(i => i.system === this.$store.getOids().local);
      if (localId) {
        ...
        useITI83(localIdString, [this.$store.getOids().mpiId])
        .then((ids) => {
    ...
    filterTable(data: DocumentReference[], filter: string): DocumentReference[] {
      ...
      return data.filter(docRef => {
        ...
        if (docRef.context?.facilityType?.coding ) {
          ...
          if (coding && coding.code) {
            const translated = getFacilityClassCodeString(coding.code, getLangStringFromLocale(locale));
        ...
        if (docRef.category && docRef.category[0].coding) {
          ...
          if (coding && coding.code) {
              const translated = getClassCodeString(coding.code, getLangStringFromLocale(locale));
        ...
        if (docRef.type && docRef.type.coding) {
          ...
          if (coding && coding.code) {
              const translated = getTypeCodeString(coding.code, getLangStringFromLocale(locale));
